...
import {
  sendFriendRequest,
  updateFriendRequest,
} from "../features/user/ContactSlice";
...
const ProfileCard = (props) => {
  ...
  const handleFollowRequest = (status) => {
    ...
    if (
      ...
      dispatch(
        updateFriendRequest({
          status,
          contactRequestId: userContacts._id,
          receiver: userId,
        }),
      );
    }
  ...
  useEffect(() => {
    const alertsToShow = [
      ...
      {
        condition:
          updateFriendRequestStatus === "failed" && updateUserContactError,
  ...
  return (
    <div
      class="card text-center mx-auto shadow profile-card"
    ...
    >
      ...
      <div class="card-body">
        ...
        <p class="card-text" style={{ fontSize: "0.7rem" }}>
          {bio}
        </p>
        ...
        {contactStatus === "friend" && (
          <button
            // to={`/chats?roomId=${chatroomId}&type=single`}
        ...
        {contactStatus === "friend" && (
          <Link href="#" class="btn btn-sm btn-danger mx-3 py-1 px-1">
            Unfollow
          </Link>
        )}
        {contactStatus === "Received" && (
        ...
        {["pending", "unknown"].includes(contactStatus) && (
